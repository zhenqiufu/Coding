...
void test() {
  ...
  while (1) {
    ...
    printf("Client 33333333333333333333333333333333 \n");
...
void test2() {
  ...
  char send_buffer[send_size] = "client 3: hello!";
  char recv_buffer[recv_size];
  while (1) {
    std::cout << "The buffer send: " << send_buffer << std::endl;
    _tcpclient.senddata(recv_buffer, send_buffer, recv_size, send_size);
    std::cout << "The buffer recived: " << recv_buffer << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
  }
}
